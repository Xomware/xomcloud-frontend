...
export class UserService {
  ...
  getUserByPermalink(permalink: string): Observable<User> {
    return this.http.get<any>(`${this.apiBaseUrl}/resolve`, {
      headers: this.authService.getAuthHeaders(),
      params: { url: `https://soundcloud.com/${permalink}` }
    }).pipe(
      catchError(error => {
        console.error(`Failed to resolve user permalink ${permalink}:`, error);
        return throwError(() => error);
      })
    );
  }
  ...
  getFollowings(userId?: number, limit: number = 50): Observable<User[]> {
    ...
    return this.http.get<{ collection: User[] }>(`${this.apiBaseUrl}/users/${id}/followings`, {
      headers: this.authService.getAuthHeaders(),
      params: { 
  ...
  getFollowers(userId?: number, limit: number = 50): Observable<User[]> {
    ...
    return this.http.get<{ collection: User[] }>(`${this.apiBaseUrl}/users/${id}/followers`, {
      headers: this.authService.getAuthHeaders(),
      params: { 
        limit: limit.toString(),
        linked_partitioning: 'true'
  ...
  followUser(userId: number): Observable<any> {
    return this.http.put(`${this.apiBaseUrl}/me/followings/${userId}`, null, {
      headers: this.authService.getAuthHeaders()
    }).pipe(
  ...
  unfollowUser(userId: number): Observable<any> {
    return this.http.delete(`${this.apiBaseUrl}/me/followings/${userId}`, {
      headers: this.authService.getAuthHeaders()
    }).pipe(
